  ...
  return (
    <div className="flex flex-col p-4 gap-4 border min-h-[100vh] bg-white overflow-y-auto">
      {data.map((user, index) => (
        <div
          key={index}
          className="flex items-center justify-start gap-4 w-full h-auto cursor-pointer transition-all hover:bg-inherit hover:text-pink-600 mt-4"
          onClick={() => navigate(`/room-chat/${user.roomChatId}`)}
        >
          <img
            className="w-16 h-16 sm:w-14 sm:h-14 rounded-full object-cover border-2 border-pink-200 shadow-sm"
            src={user?.profilePic}
            alt={user?.name}
          />
